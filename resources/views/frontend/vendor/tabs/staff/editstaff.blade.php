...
@section('content')
<div class="container mx-auto px-4 py-8" x-data="{ showForm: true, innerTab: 'info' }">
    ...
    <div class="flex gap-6">
        ...
        <div class="w-3/4 bg-white shadow rounded-2xl p-6">
            <div x-show="showForm" x-transition class="mb-6">
                ...
                <form action="{{ route('vendor.staff.update',$staffdata[0]->id) }}" method="POST"
                    ...
                    <div x-show="innerTab === 'info'" class="space-y-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="{{ old('name', $staffdata[0]->name) }}"
                                class="w-full border p-2 rounded focus:ring focus:ring-indigo-200 {{ $errors->has('name') ? 'border-red-500' : '' }}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <div class="flex gap-2">
                                ...
                                <input type="text" name="phone_number"
                                    value="{{ old('phone_number', $staffdata[0]->phone_number) }}"
                                    class="flex-1 border p-2 rounded focus:ring focus:ring-indigo-200 {{ $errors->has('phone_number') ? 'border-red-500' : '' }}">
                            </div>
                            @error('phone_number')
                            <div class="text-red-500 mt-2 error_message">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="form-group">
                                ...
                                <input type="password" name="password"
                                    class="w-full border p-2 rounded {{ $errors->has('password') ? 'border-red-500' : '' }}">
                                @error('password')
                                <div class="text-red-500 mt-2 error_message">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <select name="status" class="border p-2 rounded w-full">
                                <option value="1" {{ old('status', $staffdata[0]->status) == 1 ? 'selected' : '' }}>
                                    Active</option>
                                <option value="0" {{ old('status', $staffdata[0]->status) == 0 ? 'selected' : '' }}>
                                    Inactive</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Update Staff</button>
                    </div>
                </form>
            </div>
        </div>

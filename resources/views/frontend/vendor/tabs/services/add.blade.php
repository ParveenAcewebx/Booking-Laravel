    ...
    <div class="w-3/4 bg-white shadow rounded-2xl p-6">
        <form action="{{ route('vendor.services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" class="w-full mt-1 p-3 border rounded-md focus:ring-2 focus:ring-indigo-500" rows="3">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="currency" class="w-full mt-1 p-3 border rounded-md focus:ring-2 focus:ring-indigo-500">
                    @foreach($currencies as $code => $currency)
                        <option value="{{ $currency['symbol'] }}" {{ old('currency') == $currency['symbol'] ? 'selected' : '' }}>
                            {{ $code }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Add Service</button>
            </div>
        </form>
    </div>
</div>

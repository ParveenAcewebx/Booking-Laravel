  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <a href="{{ url('vendor/' . $vendor->id) }}" class="flex flex-col rounded-xl p-4 md:p-6 bg-white border border-gray-200 dark:bg-neutral-900 dark:border-neutral-700 hover:shadow-lg transition">
      <div class="flex items-center gap-x-4">
        <img class="rounded-full size-20 border-2 border-blue-150"
          src="{{ $vendor->thumbnail && file_exists(public_path('storage/' . $vendor->thumbnail)) 
                ? asset('storage/' . $vendor->thumbnail) 
      ...
      <p class="mt-3 text-gray-500 dark:text-neutral-500">
        {!! trim($vendor->description) === '<p><br></p>' || empty($vendor->description)
...
<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    <div class="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
    <h2 class="text-2xl font-bold md:text-4xl md:leading-tight dark:text-white">Our Services</h2>
    <p class="mt-1 text-gray-600 dark:text-neutral-400">Creative people</p>
  </div>
  <div class="grid sm:grid-cols-2 lg:grid-cols-3 items-center gap-6 md:gap-10">
@foreach ($service as $sericedetail)
  <a href="" class="flex flex-col rounded-xl p-4 md:p-6 bg-white border border-gray-200 dark:bg-neutral-900 dark:border-neutral-700 hover:shadow-lg transition">
   
      <div class="flex items-center gap-x-4 mb-3">
        <div class="inline-flex justify-center items-center size-15.5 rounded-full border-4">
          <img src="{{ $sericedetail->thumbnail ? asset('storage/'. $sericedetail->thumbnail ):asset('assets/images/no-image-available.png') }} "class="max-w-[40px]" alt="">
        </div>
        <div class="shrink-0">
          <h3 class="block text-lg font-semibold text-gray-800 dark:text-white">{{$sericedetail->name}}</h3>
        </div>
      </div>
      <p class="text-gray-600 dark:text-neutral-400">{{strip_tags($sericedetail->description)}}</p>
